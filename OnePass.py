...
def calculate_tcf(t, tcf):
    ...
    """
    adjusted_tcf = np.exp(tcf*(t/298.15)-1./(t+273.5))  # sorry; magic numbers using for now
    return adjusted_tcf
...
class OnePass:
    ...
    def create_dataframe(self, plant_df=''):
        ...
        Plant = collections.namedtuple('Plant', 'date time ph temp \
                                                conc_flow perm_flow \
                                                feed_press conc_pres perm_press \
                                                feed_cond perm_cond')
        ...
        avg_feed_tds = cpf * feed_tds * (-np.log(1 - feed_tds) / feed_tds)
        diff_pres = plant.feed_press - plant.conc_pres
        perm_osm_pres = 1.8*perm_tds/55850*0.0821*(273+plant.temp)*14.7
        net_driving_pres = plant.feed_press - diff_pres / 2 - perm_osm_pres - plant.perm_press + perm_osm_pres
        flux = plant.perm_flow * min_per_day / membrane_area
        tcf_water = calculate_tcf(water_tcf0, plant.temp)
        tcf_salt = calculate_tcf(salt_tcf0, plant.temp)
        avg_feed_osm_pres = 0.0385 * avg_feed_tds * (plant.temp + 273.15) / (1e3 - avg_feed_tds / 1e3)
        norm_salt_pass = (perm_tds/avg_feed_tds)*(plant.perm_flow*feed_tds*avg_feed_tds[0]*tcf_salt[0])/(plant.perm_flow*feed_tds[0]*avg_feed_tds*tcf_salt)
        ...
        temp = pd.DataFrame({'Feed Temp (°C)': plant.temp,
                             ...
                             'TMP (psi)': plant.feed_press - plant.perm_press,
                             ...
                             'B Value \n[STC] \n(m/s)': plant.perm_flow*perm_tds/tcf_salt/264.17/60/(membrane_area*0.0929)/(avg_feed_osm_pres - perm_tds),
